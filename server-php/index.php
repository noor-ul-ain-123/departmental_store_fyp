...
if ($uri === '/api/auth/register' && $method === 'POST') {
  ...
  if ($city === '' || $contact === '') {
    http_response_code(400);
    echo json_encode(["error" => "City and contact number are required"]);
    exit;
  }
  ...
  $pdo = db(); // ✅ IMPORTANT
  $stmt = $pdo->prepare("INSERT INTO users (name, email, password_hash, city, contact_number) VALUES (?,?,?,?,?)");
  $stmt->execute([$name, $email, $hash, $city, $contact]);
...
if ($uri === '/api/uploads' && $method === 'POST') {
  ...
  if (!isset($_FILES['file']))
    json_out(['error' => 'file_required'], 400);
  $f = $_FILES['file'];
  if ($f['error'] !== UPLOAD_ERR_OK)
    json_out(['error' => 'upload_failed', 'code' => $f['error']], 400);
...
if ($uri === '/api/orders' && $method === 'POST') {
  ...
  if (count($items) === 0 || $derived <= 0 || $clientTotal <= 0) {
    json_out(['error' => 'empty_or_invalid_cart'], 422);
  }
...
if ($uri === '/api/contact' && $method === 'POST') {
  ...
  $subject = trim((string) ($b['subject'] ?? 'General Inquiry'));
  $message = trim((string) ($b['message'] ?? ''));

  // validation
  if ($name === '' || strlen($name) < 2 || strlen($name) > 140)
    json_out(['error' => 'invalid_name'], 422);
  if (!filter_var($email, FILTER_VALIDATE_EMAIL))
    json_out(['error' => 'invalid_email'], 422);
  if (strlen($phone) > 30)
    json_out(['error' => 'invalid_phone'], 422);
  if ($subject === '' || strlen($subject) > 120)
    json_out(['error' => 'invalid_subject'], 422);
  if ($message === '' || strlen($message) < 5)
    json_out(['error' => 'invalid_message'], 422);

  // OPTIONAL: attach user_id if logged in
  $au = auth_user(); // returns jwt payload or null
  $userId = ($au && isset($au['sub'])) ? (int) $au['sub'] : null;
